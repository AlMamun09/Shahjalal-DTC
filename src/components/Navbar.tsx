...
export function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-40 text-white transition-all duration-500 ${scrolled ? 'glass shadow-lg' : 'bg-brand-black'}`}>
      <nav className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-0.5">
          ...
          {isAdmin && (
            <Link to="/admin" className="px-3 py-2 rounded-lg text-sm font-medium text-brand-gold hover:bg-brand-gold/10 transition-all flex items-center gap-1.5">
          ...
          <button onClick={toggleLang} className="text-sm px-3 py-1.5 border border-white/30 rounded-lg hover:bg-white/10 transition-all hover:border-white/50">
        ...
        <div className="lg:hidden flex items-center gap-2">
          {isAdmin && <Link to="/admin" className="text-brand-gold text-xs px-2 py-1 border border-brand-gold/30 rounded-lg">Admin</Link>}
          <button onClick={toggleLang} className="text-xs px-2 py-1 border border-white/30 rounded-lg">{i18n.language === 'bn' ? 'EN' : 'বাংলা'}</button>
